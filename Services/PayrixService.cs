...
public class PayrixService
{
    ...
    // ── Create a test vault token (POST /customers → POST /tokens) ───────────
    /// <summary>
    /// Two-step flow:
    ///   1. POST /customers  — creates a minimal test customer under the merchant
    ///   2. POST /tokens     — creates a vault token linked to that customer + card
    /// Returns (tokenId, combinedLog, errorMessage).
    /// </summary>
    public async Task<(string? tokenId, string rawJson, string? error)>
        CreateTestTokenAsync(string merchantId, string cardNumber, string expiration,
                             string? cvv = null, string? cardHolder = null)
    {
        ...
        try
        {
            ...
            var custPayload = new
            {
                ...
                first    = "Test",
                last     = "Customer",
                email    = $"test_{Guid.NewGuid():N}@sandbox.test"
            };

            var custBody = new StringContent(
                System.Text.Json.JsonSerializer.Serialize(custPayload),
                System.Text.Encoding.UTF8, "application/json");

            var custResp = await _client.PostAsync("/customers", custBody).ConfigureAwait(false);
            var custJson = await custResp.Content.ReadAsStringAsync().ConfigureAwait(false);
            log.AppendLine("── POST /customers ──");
            log.AppendLine(custJson);

            if (!custResp.IsSuccessStatusCode)
                return (null, log.ToString(), ExtractPayrixError(custJson) ?? $"Customer creation HTTP {(int)custResp.StatusCode}");

            // Parse customer ID from  { response: { data: [ { id: "..." } ] } }
            string? customerId = null;
            try
            {
                using var custDoc = System.Text.Json.JsonDocument.Parse(custJson);
                if (custDoc.RootElement.TryGetProperty("response", out var cr) &&
                    cr.TryGetProperty("data", out var cd) &&
                    cd.GetArrayLength() > 0 &&
                    cd[0].TryGetProperty("id", out var cid))
            ...
            {
                var custErr = ExtractPayrixError(custJson);
                return (null, log.ToString(), custErr ?? "Customer created but no ID returned.");
            }

            // ── Step 2: Create token ──────────────────────────────────────────
            var payment = new System.Collections.Generic.Dictionary<string, object?>
            {
                ["number"]     = cardNumber,
                ["expiration"] = expiration,
            };
            if (!string.IsNullOrWhiteSpace(cvv))        payment["cvv"]  = cvv;
            if (!string.IsNullOrWhiteSpace(cardHolder)) payment["name"] = cardHolder;

            var tokPayload = new { merchant = merchantId, customer = customerId, payment };

            var tokBody = new StringContent(
                System.Text.Json.JsonSerializer.Serialize(tokPayload, new System.Text.Json.JsonSerializerOptions
                {
                    DefaultIgnoreCondition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull
                }),
                System.Text.Encoding.UTF8, "application/json");

            var tokResp = await _client.PostAsync("/tokens", tokBody).ConfigureAwait(false);
            var tokJson = await tokResp.Content.ReadAsStringAsync().ConfigureAwait(false);
            log.AppendLine("── POST /tokens ──");
            log.AppendLine(tokJson);

            if (!tokResp.IsSuccessStatusCode)
                return (null, log.ToString(), ExtractPayrixError(tokJson) ?? $"Token creation HTTP {(int)tokResp.StatusCode}");

            var parsed = System.Text.Json.JsonSerializer.Deserialize<PayrixTokenResponse>(tokJson, JsonOptions);
            if (parsed?.Response?.Errors is { Count: > 0 })
                return (null, log.ToString(), string.Join("  |  ", parsed.Response.Errors.Select(e => e.Summary)));

            var token = parsed?.Response?.Data?.FirstOrDefault();
            if (token?.Id is null)
                return (null, log.ToString(), "Token created but no ID returned.");

            return (token.Id, log.ToString(), null);
        }
        catch (Exception ex)
        {
            return (null, log.ToString(), ex.Message);
        }
    }

    // ── Fetch vault tokens for a merchant ────────────────────────────────────
    /// <summary>
    /// Returns all vault payment tokens stored under <paramref name="merchantId"/>.
    /// Each token has an Id (to paste into the Token field) and nested card info.
    /// Returns (list, rawJson, errorMessage).
    /// </summary>
    public async Task<(List<PayrixToken> tokens, string rawJson, string? error)>
        GetTokensByMerchantAsync(string merchantId)
    {
        var json = "{}";
        try
        {
            ...
            if (_environment == PayrixEnvironment.Production)
            {
                // Production uses search header
                var request = new HttpRequestMessage(HttpMethod.Get,
                    "/tokens?page[limit]=100&page[number]=1&expand[payment][]");
                request.Headers.Add("search", $"merchant[eq]={merchantId}");
                response = await _client.SendAsync(request).ConfigureAwait(false);
            }
            else
            {
                // Sandbox accepts search in query string
                response = await _client.GetAsync(
                    $"/tokens?search[merchant][eq]={Uri.EscapeDataString(merchantId)}&page[limit]=100&page[number]=1&expand[payment][]")
            ...
            if (!response.IsSuccessStatusCode)
                return ([], json, $"HTTP {(int)response.StatusCode} — {response.ReasonPhrase}");

            var parsed = JsonSerializer.Deserialize<PayrixTokenResponse>(json, JsonOptions);
            ...
            if (data.Count == 0)
            {
                ...
                if (errors is { Count: > 0 })
                    return ([], json, string.Join("  |  ", errors.Select(e => e.Summary)));
                return ([], json, null);   // No tokens found — not an error
            }

            // Client-side exact-match filter: sandbox may ignore the search param
            // and return tokens from all merchants — keep only the ones that actually
            // belong to the requested merchant.
            var filtered = data
                .Where(t => string.IsNullOrEmpty(t.Merchant) ||
                            string.Equals(t.Merchant, merchantId, StringComparison.OrdinalIgnoreCase))
                .ToList();

            // If the filter removed everything it means the server returned unrelated data;
            // fall back to the unfiltered list so the user still sees something.
            var result = filtered.Count > 0 ? filtered : data;
    ...
    // ── Post a transaction with Level 2/3 line item detail ───────────────────

    /// <summary>
    /// POSTs a new transaction to Payrix /txns with <c>lineItemDetailIndicator=1</c>
    /// and the supplied line items array.  Use this to test Level 2/Level 3 enhanced data.
    /// </summary>
    /// <param name="merchantId">The Payrix merchant ID to charge.</param>
    /// <param name="token">Payment token (card/ACH token from Payrix vault).</param>
    /// <param name="totalCents">Transaction total in cents (e.g. 1099 = $10.99).</param>
    /// <param name="taxCents">Tax amount in cents.</param>
    /// <param name="type">Transaction type: 1=Sale, 2=Authorize, 3=Capture.</param>
    /// <param name="description">Optional transaction description.</param>
    /// <param name="order">Optional order/invoice reference.</param>
    /// <param name="items">Line items to attach.  Prices/totals must be in cents.</param>
    /// <returns>(created transaction, raw JSON, error message)</returns>
    public async Task<(Transaction? transaction, string rawJson, string? error)>
        CreateTransactionWithLineItemsAsync(
            string merchantId,
            string token,
            decimal totalCents,
            decimal taxCents,
            ...
            IReadOnlyList<TransactionItem> items)
    {
        ...
        try
        {
            // Build item payload — only include non-null/non-empty fields.
            // lineItemDetailIndicator is per-item (Worldpay CNP spec):
            //   0 = Normal  1 = Normal last  2 = Credit  3 = Credit last
            //   4 = Payment  5 = Payment last
            // Auto-assign "last" variant on the final item if the caller left it null.
            var lastIdx = items.Count - 1;
            var itemPayloads = items.Select((it, idx) =>
            {
                // If the caller hasn't explicitly set an indicator, default to 0 (Normal)
                // and upgrade the last item to 1 (Normal last).
                int indicator = it.LineItemDetailIndicator
                    ?? (idx == lastIdx ? 1 : 0);
                ...
                return new
                {
                    ...
                    itemSequenceNumber       = it.ItemSequenceNumber ?? (idx + 1),
                    taxAmount                = it.TaxAmount,
                    taxRate                  = it.TaxRate,
                    taxIncludedInTotal       = it.TaxIncludedInTotal,
                    lineItemDetailIndicator  = (int?)indicator
                };
            }).ToList();
            ...
            var resp = await _client.PostAsync("/txns", body).ConfigureAwait(false);
            json = await resp.Content.ReadAsStringAsync().ConfigureAwait(false);
            ...
            var parsed = JsonSerializer.Deserialize<PayrixResponse>(json, JsonOptions);
            if (parsed?.Response?.Errors is { Count: > 0 })
                return (null, json, string.Join("  |  ", parsed.Response.Errors.Select(e => e.Summary)));

            var txn = parsed?.Response?.Data?.FirstOrDefault();
            return (txn, json, txn == null ? "POST succeeded but no transaction returned." : null);
        }
        catch (Exception ex)
        {
            return (null, json, $"POST error: {ex.Message}");
        }
    }
